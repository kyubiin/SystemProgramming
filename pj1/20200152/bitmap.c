...
{
  // 비트맵을 `size` 비트만큼 확장, 새 비트들은 0으로 초기화
  // 매개변수:
  // - bitmap: 확장할 비트맵
  // - size: 비트맵에 추가할 비트의 수
  // 반환값: 확장된 비트맵이며, 메모리 할당에 실패할 경우 NULL을 반환

  if (size <= 0)
  {
    // 확장할 크기가 0 이하인 경우, 변경 없이 원본 비트맵을 반환
    return bitmap;
  }

  size_t old_size = bitmap_size(bitmap);                           // 원래 크기를 저장
  size_t new_size = old_size + size;                               // 새로운 크기를 계산
  elem_type *new_bits = realloc(bitmap->bits, byte_cnt(new_size)); // 확장된 비트맵을 위해 메모리를 재할당
  ...
  bitmap->bits = new_bits;    // 비트맵의 비트 포인터를 업데이트
  bitmap->bit_cnt = new_size; // 비트맵의 크기를 업데이트

  // 새 비트들을 0으로 초기화
  bitmap_set_multiple(bitmap, old_size, size, false);

  return bitmap; // 확장된 비트맵을 반환
}
